...
func GeoLocation(key, place string, returnModel chan PositionStackModel) {
...
func GetWeather(key, lat, lon string, cnt string, returnModel chan OpenWeatherModel) {
	...
	requestData.Add("appid", cnt)
	requestData.Add("units", "metric")

	request, err := http.NewRequest("GET", "https://api.openweathermap.org/data/2.5/onecall?"+requestData.Encode(), nil)
	if err != nil {
